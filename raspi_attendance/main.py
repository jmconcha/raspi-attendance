...
def timein(stud_id, sel_subj_id):
    stud_info = get_student(stud_id)
    ...
    conn, cur = open_conn()
    cur.execute(
        f"INSERT INTO attendance (subject, student, time_in) VALUES ({sel_subj_id}, {stud_id}, '{timein_str}')")
    conn.commit()
    conn.close()
    ...
    def scan_fingerprint(frame, sel_subj_id):
        ...
        def threaded_scan():
            ...
            if get_fingerprint():
                ...
                stud_info = timein(finger.finger_id, sel_subj_id)
                clear_frame(frame)
